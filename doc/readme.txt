...
把POJO包和类直接复制进工程中,而不是通过依赖manage-pojo工程.
因为这个工程自己做在cjShop-back工程里了,该back工程总体是个WAR包,不是jar包,无法依赖.
因为后端不是高并发,不需要分布式.
...
注意:
    cjshop中,到目前的笔记为止(4.商品列表展示),共涉及到三个项目:
    cjShop-parent
    cjShop-common
    cjShop-back

    其中,前两个只是为了打jar包给其他项目引用的,所以只要maven-install即可,不用使用web容器启动他们.
    而第三个才是真正的web项目,需要用tomcat启动,端口暂定为8080.
